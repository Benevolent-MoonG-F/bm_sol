// Current Model of Lottery Contract in Native rust from Collotto Repo- Changes done in repo. 
// full repo at https://github.com/prakhar1234/Colatto.git
...
pub enum LotteryInstruction {
    ...
    /// 3. `[writable]` Colatto holding wallet account (must be a system account)
    /// 4. `[wirtable]` User's Colatto Lifetime Ticket Account
    /// 5. `[signer]` Colatto Lifetime Ticket mint authority
    /// 6. `[writable]` Colatto Lifetime Ticket Mint
    /// 7. `[]` Rent sysvar
    ...
    /// 2. `[writable, signer]` Colatto holding wallet account (must be a system account)
    ...
    /// Update Colatto wallets in lottery data account
    /// Accounts expected by this instruction:
    ///
    /// 0. `[writable, signer]` Lottery data account
    UpdateColattoWallets {
        holding_wallet: Pubkey,
        rewards_wallet: Pubkey,
    },
}
...
/// Creates a `UpdateColattoWallets` instruction
pub fn update_colatto_wallets(
    program_id: &Pubkey,
    holding_wallet: &Pubkey,
    rewards_wallet: &Pubkey,
    lottery_authority: &Pubkey,
    ...
    check_program_account(program_id)?;
    let data = LotteryInstruction::UpdatecolattoWallets {
        holding_wallet: *holding_wallet,
        rewards_wallet: *rewards_wallet,
