...
accounts needed:
user account
my token account for the play token
token account for entry token
PDA to hold entry for each game 
system program 
token program 
Charity Account
BM Account 
...
price feed data- Pyth fed into each playable token  set up in State.rs
time - cronjob data pulled from Clockworks
start price struct for each token 
close price struct for each token



PYTH- sample to use for price feeds and use
use std::mem::size_of;
use anchor_lang::prelude::*;
use solana_program::account_info::AccountInfo;
